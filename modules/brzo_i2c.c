...
//		uint16_t fr_sel = round(SCL_frequency_KHz / 100.0);
		...
		// http://stackoverflow.com/questions/2570934/c-code-to-round-numbers
		uint16_t fr_sel = (uint16_t)(SCL_frequency_KHz / 100.0 < 0 ? (SCL_frequency_KHz / 100.0) : (SCL_frequency_KHz / 100.0 + 0.5));

		//	if (F_CPU == 160000000) {
		if (system_get_cpu_freq() == 160000000) {
...
//	scl_bitmask = (uint16_t)(1 << scl);
	...
	sda_bitmask = (uint16_t)(1 << I2C_MASTER_SDA_GPIO); // mrv
	scl_bitmask = (uint16_t)(1 << I2C_MASTER_SCL_GPIO); // mrv
